...
static CString CaptureRoot()
{
	...
	CString out;
	out.Format("%s\\automation_captures\\", dir.GetString());
	return out;
}
...
void COpenScrapeView::LoadCapturesFromDisk()
{
	...
	// (automation_captures\<N>\). If the stable folder is empty, adopt the most-recently
...
void COpenScrapeView::DebugRegionToFolder(const CString &region_name)
{
	...
	if (!transform.IsEmpty() && transform[0] == 'A' && theApp.m_TableMapDlg != NULL) {
		...
		dcorr_info = theApp.m_TableMapDlg->GetLastDecimalCorrectionInfo();
	}
	...
	// Decimal-correction post-processing details (AutoOcr only).
	if (!dcorr_info.IsEmpty()) info += dcorr_info;
