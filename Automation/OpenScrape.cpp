...
BOOL COpenScrapeApp::InitInstance()
{
	...
	// AUTOMATION MAPS LIVE IN THEIR OWN SCHEMA -- set this FIRST, before anything can open a DB
	// connection (registry.cpp lazily constructs CTablemapDB too). The search_path is applied when
	// the connection opens, so a connection made before this call would be pointed at public for
	// the life of the process.
	// Until 2026-07-20 nothing set this: the automation schema existed but was never used, and
	// Automation.exe was reading and writing HISS's own tablemaps/tm_* tables.
	...
	// Tablemaps are persisted in the PostgreSQL "hiss" database (automation schema, set above).
