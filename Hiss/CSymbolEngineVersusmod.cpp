...
void CSymbolEngineVersus::ClearWinTieLosData() {
	_vsprwin = _vsprtie = _vsprlos = 1;
	_vsprwinhi = _vsprtiehi = _vsprloshi = 1;
	_vsprwinti = _vsprtieti = _vsprlosti = 1;
	_vsprwinlo = _vsprtielo = _vsprloslo = 1;
	_vsprwinhinow = _vsprtiehinow = _vsprloshinow = 1;
	_vsprwintinow = _vsprtietinow = _vsprlostinow = 1;
	_vsprwinlonow = _vsprtielonow = _vsprloslonow = 1;
  for (int i=0; i<(kNumberOfCardsPerDeck - 1); i++) 	{
	  for (int j=i+1; j<kNumberOfCardsPerDeck; j++) {
      _n_win_against_hand[i][j] = 1;
      _n_tie_against_hand[i][j] = 1;
      _n_los_against_hand[i][j] = 1;
    }
  }
}
...
bool CSymbolEngineVersus::GetCounts() {
  ...
  ClearWinTieLosData();
  int tmp = p_engine_container->symbol_engine_active_dealt_playing()->nopponentsdealt();

  for (int i = 0; i < (vs1x1 ? 1 : p_engine_container->symbol_engine_active_dealt_playing()->nopponentsdealt()); i++)
  {
