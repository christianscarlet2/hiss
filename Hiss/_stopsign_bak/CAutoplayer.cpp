...
bool CAutoplayer::ExecutePrimaryFormulasIfNecessary() {
	...
	{
		const char *why = "";
		if (!SeatIsStableForActing(&why)) {
			APTrace("ExecutePrimaryFormulas -> STOP: seat not stable for acting (%s)", why);
			write_log(k_always_log_errors,
				"[AutoPlayer] STAND DOWN: %s -- refusing to act on an unstable scrape\n", why);
