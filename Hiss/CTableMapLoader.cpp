...
void CTableMapLoader::SwitchTablemapForGameTypeIfNeeded() {
  ...
  // Load the Omaha (4-card) map only when the table is detected Omaha; the Hold'em (2-card) map
  // otherwise. (Reverted the "always-Omaha-map" experiment: forcing the 4-card map onto a NLH table --
  // e.g. a NLH cash game running alongside a PLO tournament -- froze it / mis-scraped. The PLO preflop
  // fix is the master f$preflop game-type DISPATCH, which does not need the always-Omaha map.) [Emrald:
  // "NLH detected as PLO, no decisions"]
